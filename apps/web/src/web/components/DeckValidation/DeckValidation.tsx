...
export function DeckValidation({
  ...
  if (compact) {
    return (
      <div className="deck-validation deck-validation--compact">
        ...
        <span className="deck-validation__count">
          {totalCards}/60
        </span>
      </div>
    );
  ...
  return (
    <div className={`deck-validation ${isValid ? 'deck-validation--valid' : 'deck-validation--invalid'}`}>
      ...
      {showBreakdown && (
        <div className="deck-validation__breakdown">
          <div className="deck-validation__breakdown-item">
            <span className="deck-validation__breakdown-value">{breakdown.pokemon}</span>
            <span className="deck-validation__breakdown-label">Pokémon</span>
          </div>
          <div className="deck-validation__breakdown-item">
            <span className="deck-validation__breakdown-value">{breakdown.trainer}</span>
            <span className="deck-validation__breakdown-label">Trainers</span>
          </div>
          <div className="deck-validation__breakdown-item">
            <span className="deck-validation__breakdown-value">{breakdown.energy}</span>
          ...
          <div className="deck-validation__breakdown-item deck-validation__breakdown-item--highlight">
            <span className="deck-validation__breakdown-value">{breakdown.basicPokemon}</span>
        ...
        <div className="deck-validation__errors">
          ...
          <ul className="deck-validation__list">
            {errors.map((error, index) => (
              <li key={`${error.code}-${index}`} className="deck-validation__item deck-validation__item--error">
                <span className="deck-validation__icon">{'✗'}</span>
                <span className="deck-validation__message">{error.message}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="deck-validation__warnings">
          ...
          <ul className="deck-validation__list">
            {warnings.map((warning, index) => (
              <li key={`${warning.code}-${index}`} className="deck-validation__item deck-validation__item--warning">
                <span className="deck-validation__icon">{'⚠'}</span>
                <span className="deck-validation__message">{warning.message}</span>
              </li>
            ))}
          </ul>
        </div>
