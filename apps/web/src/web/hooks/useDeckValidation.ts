...
import type { DeckCard, DeckFormat, DeckValidation, ValidationError, ValidationWarning } from '../types/deck';
...
function isBasicPokemon(card: Pokemon.Card): boolean {
  return card.supertype === 'Pokémon' && (card.subtypes?.includes('Basic') ?? false);
}
...
export function validateDeck(
  ...
  for (const deckCard of deckCards) {
    ...
    if (rules.legalSets?.length && card.set?.id && !rules.legalSets.includes(card.set.id)) {
      errors.push({
        code: 'ILLEGAL_SET',
  ...
  // Check copy limits
  for (const [cardName, { count, cardIds }] of Object.entries(copyCountByName)) {
