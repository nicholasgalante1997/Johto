...
export function loadConfig(): CronServiceConfig {
  return {
    ...
    logLevel: (process.env.CRON_LOG_LEVEL as CronServiceConfig['logLevel']) ?? 'info',
    metricsEnabled: process.env.CRON_METRICS_ENABLED !== 'false',
    databasePath: process.env.DATABASE_PATH ?? './database/pokemon-data.sqlite3.db',
    ...
    backupRetention: {
      ...
      monthlyRetention: parseInt(process.env.BACKUP_MONTHLY_RETENTION ?? '3', 10),
    ...
    notifications: {
      ...
      minSeverity: (process.env.NOTIFICATION_MIN_SEVERITY as 'info' | 'warn' | 'error' | 'critical') ?? 'error',
    },
    sync: {
...
export function validateConfig(config: CronServiceConfig): string[] {
  ...
  if (!config.notifications.slackWebhookUrl && !config.notifications.alertEmail) {
